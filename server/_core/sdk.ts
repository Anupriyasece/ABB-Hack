...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    try {
      ...
      const dbUser = await db.getUserByOpenId(mockUser.openId);
      if (dbUser) {
        return dbUser as AuthenticatedUser;
      }
    } catch (error) {
      console.error("[Auth] Failed to upsert/get mock user in database:", error);
    }
    return mockUser;
  }
}
...
function buildCronUser(
  ...
  return {
    ...
    name: userInfo.name || "ABB Scheduled Task",
    email: null,
    loginMethod: null,
    role: "user",
    createdAt: now,
